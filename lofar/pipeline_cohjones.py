...
do_cohjones=str(environ['cohjones_cal'])
...
if(do_initial=='True'):
	...
	for s in range(nsb):
		...
		for i in range(lstart,lend+1,3):
			cmdstr='NDPPP '+sd+'flag_LB.ndppp msin='+od+'L'+str(i)+'_SB%03d'%csb+ending+' msout='+wd+'L'+str(i)+'_SB%03d_uv.dppp.CRS.MS'%csb+' > '+wd+'L'+str(i)+'_SB%03d'%csb+ending+'.flagLBHigh.log'
			print(cmdstr)
	...
	for i in range(nsb):
		csb = sb + i
		fn = od+'L'+str(lampcal)+'_SB%03d'%csb+ending
...
if(do_cohjones=='True'):
	ending='_uv.dppp.CRS.MS'
	# CohJones based phase and amplitude calibration
	print('Doing CohJones cal.')
	ps=[]
	for s in range(nsb):
		csb = sb + s
		for i in range(lstart,lend+1,3):
			cmdstr='CohJones.py --ms='+wd+'L'+str(i)+'_SB%03d'%csb+ending+' --SkyModel='+skymodel+' --DoBar=0 --timestep=5 --TChunk=0.1 --NCPU='+str(maxthread)
			print(cmdstr)
			p = Popen(cmdstr, shell=True)
			ps.append(p)
			waitfunc(ps)
