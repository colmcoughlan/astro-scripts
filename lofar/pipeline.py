...
for s in range(nsb):
	csb = sb + s
	for i in range(lstart,lend+1,3):
		cmdstr='sagecal -d '+wd+'L'+str(i)+'_SB%03d'%csb+ending+' -s '+skymodel+' -c '+cluster_file+' -p '+wd+'L'+str(i)+'_SB%03d_'%csb+'sagecal_solutions.txt -I CORRECTED_DATA -t 30'
		p = Popen(cmdstr, shell=True)
		ps.append(p)
...
# Create virtual concatenation for awimager
