...
for s in range(nsb):
	csb = sb + s
	for i in range(lstart,lend+1,3):
		cmdstr='NDPPP '+sd+'average_fake.dppp msin='+od+'L'+str(i)+'_SB%03d'%csb+ending+' msout='+wd+'L'+str(i)+'_SB%03d'%csb+ending+' > '+od+'L'+str(i)+'_SB%03d'%csb+ending+'.corr_to_remote.ndppp.log'
		print(cmdstr)
		p = Popen(cmdstr, shell=True)
		ps.append(p)
		if(len(ps)==maxthread):
